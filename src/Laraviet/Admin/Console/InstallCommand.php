...
class InstallCommand extends Command
{
    ...
    {
        if ($this->confirm('Do you want publish the laraviet/admin\'s migrations ?')) {
            $this->call('admin:migration');
        }
        ...
        if ($this->confirm('Do you want run the laraviet/admin\'s seeders now ?')) {
            $this->call('admin:seed');
        }

        if ($this->confirm('Do you want publish configuration files from laraviet/admin package ?')) {
            $this->call('vendor:publish', [
                '--provider' => 'Laraviet\Admin\AdminServiceProvider',
                ['--tag' => ['config']],
            ]);
        }

        if ($this->confirm('Do you want publish assets from laraviet/admin package ?')) {
            $this->call('vendor:publish', [
                '--provider' => 'Laraviet\Admin\AdminServiceProvider',
                ['--tag' => ['assets']],
            ]);
